...
export function ArrangeView({ imageUrl, objects, segmenting: initialSegmenting, onBack, onInstructionApplied }: Props) {
  ...
  const handleMouseDown = useCallback((e: React.MouseEvent, objId: string) => {
    ...
    const rect = containerRef.current?.getBoundingClientRect();
    if (!rect) return;
    setDragOffset({ x: ((e.clientX - rect.left) / rect.width) * 100 - pos.x, y: ((e.clientY - rect.top) / rect.height) * 100 - pos.y });
    setDraggingId(objId);
  ...
  const handleMouseUp = useCallback(async () => {
    ...
    try {
      ...
      if (newUrl) {
        setCurrentImage(newUrl); onInstructionApplied(newUrl);
        setRescanning(true);
        try {
  ...
  return (
    <div style={{
      ...
      <div style={{ flex: 1, display: 'flex', overflow: 'hidden', minHeight: 0 }}>
        ...
        <div style={{ flex: 1, position: 'relative', overflow: 'hidden', background: 'var(--garden-black)' }}>
          <div ref={containerRef} style={{
            ...
            <img src={currentImage} alt="Garden arrange" style={{
            ...
            {!isWorking && currentObjects.map(obj => {
              ...
              return (
                <div key={obj.id} onMouseDown={e => handleMouseDown(e, obj.id)} style={{
                  position: 'absolute', left: `${pos.x}%`, top: `${pos.y}%`,
                  transform: `translate(-50%, -50%) ${isDragging ? 'scale(1.08)' : 'scale(1)'}`,
                  cursor: 'grab', zIndex: isDragging ? 100 : 10,
                  transition: isDragging ? 'none' : 'left 0.1s, top 0.1s, transform 0.15s var(--ease-spring)',
                }}>
                  <div style={{
                    background: isDragging ? 'rgba(122,182,72,0.9)' : isHighlighted ? 'rgba(122,182,72,0.15)' : 'rgba(15,26,18,0.85)',
                    border: isDragging || isHighlighted ? '1.5px solid var(--green-400)' : '1px solid rgba(122,182,72,0.15)',
                    borderRadius: 'var(--radius-xl)', padding: '5px 10px',
                    display: 'flex', alignItems: 'center', gap: 5, whiteSpace: 'nowrap',
                    backdropFilter: 'blur(8px)',
                    boxShadow: isDragging ? '0 4px 20px rgba(122,182,72,0.4)' : isHighlighted ? 'var(--shadow-glow)' : 'var(--shadow-md)',
            ...
            {applying && (
              <div style={{
                ...
                <div style={{ display: 'flex', gap: 10 }}><span className="dot" /><span className="dot" /><span className="dot" /></div>
                <p style={{ fontSize: 15, color: 'var(--text-primary)', fontWeight: 300, fontFamily: 'var(--font-display)', fontStyle: 'italic' }}>
                  {t('arrange.moving')} {applyingLabel}...
                </p>
              </div>
            )}
        ...
        <div className="marble-green" style={{
          ...
          <div style={{
            padding: '12px 16px', borderBottom: '1px solid var(--garden-border)',
            ...
            <span style={{ fontSize: 11, fontWeight: 700, color: 'var(--green-400)', letterSpacing: '0.08em' }}>
              {t('arrange.objects')} ({currentObjects.length})
            </span>
          </div>
          ...
          <div style={{
            ...
            {currentObjects.map((obj, i) => (
              <div key={obj.id}
                ...
                <span style={{ fontSize: 17, flexShrink: 0, filter: 'drop-shadow(0 1px 2px rgba(0,0,0,0.3))' }}>{obj.emoji}</span>
                <span style={{
                  flex: 1, fontSize: 11, color: 'var(--text-secondary)', fontWeight: 500,
                  overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap',
                }}>{obj.label}</span>
                <button onClick={() => handleRemoveObject(obj)} disabled={isWorking} style={{
                  background: 'transparent', border: 'none', color: 'var(--text-muted)',
                  fontSize: 15, cursor: isWorking ? 'not-allowed' : 'pointer',
                  padding: '0 3px', lineHeight: 1, flexShrink: 0, transition: 'color 0.15s',
          ...
          <div style={{
            ...
            <button onClick={handleRescan} disabled={isWorking} className="btn-ghost" style={{
              ...
              {rescanning ? (
                <>
                  ...
                  <span className="dot" style={{ width: 4, height: 4 }} />
                  <span className="dot" style={{ width: 4, height: 4 }} />
                  <span style={{ marginLeft: 4 }}>{t('arrange.rescanning')}</span>
                </>
              ) : (
                <>
                  <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
      ...
      {/* Status bar */}
      <div style={{
        padding: '8px 24px', height: 36, borderTop: '1px solid var(--garden-border)',
        ...
        {applying ? (
          <>
            <span className="dot" style={{ width: 4, height: 4 }} />
            <span style={{ color: 'var(--text-secondary)' }}>{t('arrange.moving')} {applyingLabel}...</span>
          </>
        ) : rescanning ? (
          <>
            <span className="dot" style={{ width: 4, height: 4 }} />
            <span style={{ color: 'var(--text-secondary)' }}>{t('arrange.rescanning2')}</span>
          </>
        ) : initialSegmenting ? (
          <>
            <span className="dot" style={{ width: 4, height: 4 }} />
            <span style={{ color: 'var(--text-secondary)' }}>{t('arrange.identifying')}</span>
          </>
        ) : (
          <>
            <div style={{
              width: 5, height: 5, borderRadius: '50%', background: 'var(--green-400)',
              boxShadow: '0 0 4px var(--green-glow)',
            }} />
            <span>{currentObjects.length} {t('arrange.identified')}</span>
          </>
        )}
      </div>
    </div>
