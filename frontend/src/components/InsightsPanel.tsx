    ...
    <div className="marble-green" style={{
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <span style={{ fontSize: 12, fontWeight: 700, color: 'var(--green-400)', letterSpacing: '0.08em' }}>
            {t('insights.title')}
          </span>
        </div>
        ...
        {result && !refreshing ? (
          <>
            ...
            <div style={{
              ...
              <div>
                <p style={{ fontSize: 11, fontWeight: 700, color: 'var(--text-primary)', marginBottom: 3, letterSpacing: '0.06em' }}>
                  {t('insights.harmony')}
                </p>
            ...
            {result.suggestions.map((s, i) => (
              <div key={i} style={{
                ...
                <div>
                  ...
                  <p style={{ fontSize: 12, color: 'var(--text-secondary)', lineHeight: 1.6 }}>{s}</p>
                </div>
              </div>
            ))}
            ...
            {result.cornerNote && (
              <div style={{
                ...
                <p style={{ fontSize: 12, color: 'var(--text-tertiary)', lineHeight: 1.6, fontStyle: 'italic' }}>
                  {result.cornerNote}
                </p>
              </div>
            )}
            ...
            {suggestedPlacements.length > 0 && (
              <div style={{
                ...
                <div style={{
                  padding: '10px 14px', background: 'var(--garden-card)',
                  ...
                  }}>{t('insights.suggestedObjects')}</span>
                </div>
                <div style={{
                  ...
                  {suggestedPlacements.map((item, i) => (
                    ...
                    >
                      <div style={{ fontSize: 24, textAlign: 'center', lineHeight: 1, filter: 'drop-shadow(0 2px 4px rgba(0,0,0,0.3))' }}>
                        {item.emoji}
                      </div>
                      <p style={{ fontSize: 10, fontWeight: 600, color: 'var(--text-primary)', textAlign: 'center', lineHeight: 1.3 }}>
                        {item.name}
                      </p>
                      <p style={{ fontSize: 9, color: 'var(--text-muted)', textAlign: 'center', lineHeight: 1.3, marginBottom: 3 }}>
                        {item.description}
                      </p>
                      <button
                        onClick={() => onInstruction(item.instruction)}
                        className="btn-ghost"
                        style={{ padding: '4px 0', fontSize: 10, fontWeight: 600, width: '100%' }}
                      >
                        {t('insights.place')}
                      </button>
                    </div>
          ...
          <div style={{
            ...
            }}>{t('insights.refreshing')}</p>
          </div>
        ) : (
          <div style={{ textAlign: 'center', paddingTop: 60, animation: 'fadeIn 0.5s ease' }}>
            <div style={{
            ...
            <p style={{ fontSize: 13, color: 'var(--text-tertiary)', lineHeight: 1.7, padding: '0 12px' }}>
              {t('insights.emptyText')}{' '}
              <em style={{ color: 'var(--green-400)', fontWeight: 500 }}>{t('insights.emptyAction')}</em>
              {' '}{t('insights.emptyEnd')}
            </p>
          </div>
        )}
      ...
      <div style={{
        ...
        {result && (
          <button
            ...
            style={{
              ...
              ...(segmenting ? { background: 'var(--garden-card)', color: 'var(--green-400)', boxShadow: 'none' } : {}),
            }}
          >
            {segmenting ? (
              <>
                ...
                <span className="dot" style={{ width: 5, height: 5 }} />
                <span className="dot" style={{ width: 5, height: 5 }} />
                <span style={{ marginLeft: 4 }}>{t('insights.scanning')}</span>
              </>
            ) : (
              <>
                {t('insights.arrangeObjects')}
        ...
        >
          ...
          <div style={{ textAlign: 'left' }}>
            <div style={{ fontSize: 12, fontWeight: 600 }}>{t('insights.flowMode')}</div>
            <div style={{
              fontSize: 10, color: result ? 'var(--text-muted)' : 'var(--text-ghost)',
              fontStyle: 'italic',
            }}>{t('insights.flowSub')}</div>
          </div>
        </button>
      </div>
    </div>
